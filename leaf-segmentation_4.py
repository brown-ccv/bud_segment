...
d = 'sample41_batch11_40_41_DE_stack_ok'
...
for f in range(len(frames)):
    ...
    if var == 0.99:
        n_leaf = int(area_leaf/30)
        n_shell = int(area_shell/60)
        n_background = int(n_shell/2)
    if var == 0.95:
        n_leaf = int(area_leaf/15)
        n_shell = int(area_shell/30)
        n_background = int(n_shell)
    if var == 0.90:
        n_leaf = int(area_leaf/6)
        n_shell = int(area_shell/12)
