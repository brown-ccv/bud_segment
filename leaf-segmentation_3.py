...
d = 'Viburnum_batch12_sampleT6_redo_cropped'
...
for f in range(len(frames)):
    ...
    if var == 0.99:
        n_leaf = int(area_leaf/20)
        n_shell = int(area_shell/40)
        n_background = int(n_shell/2)
    if var == 0.95:
        n_leaf = int(area_leaf/10)
        n_shell = int(area_shell/20)
        n_background = int(n_shell/1)
    if var == 0.90:
        n_leaf = int(area_leaf/4)
        n_shell = int(area_shell/8)
