...
for f in range(len(frames)):
    ...
    if var == 0.99:
        n_leaf = int(area_leaf/70)
        n_shell = int(area_shell/140)
        n_background = int(n_shell/2)
    if var == 0.95:
        n_leaf = int(area_leaf/35)
        n_shell = int(area_shell/70)
        n_background = int(n_shell/1)
    if var == 0.90:
        n_leaf = int(area_leaf/14)
        n_shell = int(area_shell/28)
